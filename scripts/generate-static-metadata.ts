...
function fallbackFor(page: PageMeta) {
  ...
  const smsConsentContent = page.path === '/contact' ? `<section id="sms-consent"><h2>Optional text message choices</h2><p>Marketing text messages: I agree to receive marketing text messages from LYCORE GROUP LLC (LYCORE) about its services, offers and updates. Message frequency varies. Message and data rates may apply. Reply STOP to opt out or HELP for help. This optional consent is not required to submit my request.</p><p>One-time verification codes: If I request a LYCORE account or identity verification code, I agree to receive one-time verification text messages for that request only. Message frequency is as requested. Message and data rates may apply. Reply STOP to opt out or HELP for help. This does not sign me up for marketing texts and is not required to submit my request. <a href="/terms#sms">SMS Terms</a> and <a href="/privacy-policy#sms">Privacy Policy</a>.</p></section>` : '';
  ...
  const sectionsHtml = (sections?: { heading: string; paragraphs: string[]; list?: string[] }[]) => (sections || []).map(section => `<h2>${esc(section.heading)}</h2>${section.paragraphs.map(text => `<p>${esc(text)}</p>`).join('')}${section.list ? `<ul>${section.list.map(item => `<li>${esc(item)}</li>`).join('')}</ul>` : ''}`).join('');
  const detailContent = detail ? `<h2>What this solves</h2><p>${esc(detail.problem)}</p>${list('What we build', detail.builds)}${playbook ? playbookContent : list('How it works', detail.workflow)}${sectionsHtml(detail.sections)}<h2>What stays under your control</h2><p>${esc(detail.control)}</p>` : guide ? `<h2>Direct answer</h2><p>${esc(guide.answer)}</p>${list('A practical workflow', guide.steps)}${sectionsHtml(guide.sections)}${list('Trade-offs and common mistakes', guide.tradeoffs)}` : '';
  ...
  return `<main class="seo-fallback" aria-label="${esc(page.h1)}">
        <section>
          ...
          ${industryItems}
          ${detailContent}${evidenceContent}${standardsContent}${operationsContent}${smsConsentContent}${directoryContent}${relatedContent}
          ${
            faqItems.length
