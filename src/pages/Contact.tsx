  ...
  return (
    <main className="relative overflow-hidden px-6 pb-14 pt-24 md:pt-28">
      <div
        ...
        style={{ background: 'radial-gradient(circle, #e8a54b, transparent 70%)' }}
      />
          ...
          <div className="mt-6 flex flex-wrap gap-x-6 gap-y-3 text-sm text-stone-300">
            {reviewPoints.map((point, index) => (
              <span key={point} className="inline-flex items-center gap-2">
                <CheckCircle2 className="h-4 w-4" style={{ color: accentFor(index) }} aria-hidden="true" />
                {point}
              </span>
            ))}
          </div>
            ...
            <div className="p-7 md:p-8">
              ...
              <div className="py-7">
                ...
                <a href={`mailto:${site.email}`} className="group flex items-center justify-between gap-4 rounded-2xl bg-white/[0.055] p-4 transition-colors hover:bg-white/[0.09] focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-white">
                  <span className="flex min-w-0 items-center gap-3">
                    <Mail className="h-5 w-5 shrink-0" style={{ color: accentFor(0) }} aria-hidden="true" />
              ...
              <div className="space-y-4 border-t border-white/10 pt-6 text-sm font-light text-stone-300">
                <p className="flex items-start gap-3">
                  <Building2 className="mt-0.5 h-4 w-4 shrink-0" style={{ color: accentFor(1) }} aria-hidden="true" />
                  <span>{site.legalName}</span>
                </p>
                <p className="flex items-start gap-3 leading-relaxed">
                  <MapPin className="mt-0.5 h-4 w-4 shrink-0" style={{ color: accentFor(2) }} aria-hidden="true" />
